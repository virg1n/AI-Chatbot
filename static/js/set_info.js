...
//   });
// });
// --- Config: same-origin API base (works on any host/port) ---
const API_BASE = (window.API_BASE ?? "").replace(/\/+$/, "");

// Join helper: accepts "/set_info" or "set_info"
...
function setBusy(isBusy) {
  ...
  if (busy) busy.style.display = isBusy ? "inline" : "none";
}
...
function showStatus(message, type = "info") {
  ...
  status.textContent = message;
  status.className = ""; // reset
  status.classList.add(type); // style .info / .success / .error in CSS
}

function showResult(json) {
  const pre = document.getElementById("result");
  if (!pre) return;
  pre.textContent = typeof json === "string" ? json : JSON.stringify(json, null, 2);
}

async function jsonFetch(pathOrUrl, options) {
  ...
  try {
    ...
    // ignore parse errors (server may return text)
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const payload = buildPayload(form);
    if (!validateIdentifier(payload)) {
      showStatus("Provide phone_number OR (first_name and last_name).", "error");
      return;
    }

    setBusy(true);
    try {
      const data = await jsonFetch("/set_info", {
        body: JSON.stringify(payload),
      });
      ...
      const summary = (() => {
        ...
        const who =
          p.first_name || p.last_name || p.phone_number
            ? ` · person: ${[p.first_name, p.last_name].filter(Boolean).join(" ")}${p.phone_number ? ` (${p.phone_number})` : ""}`
            : "";
        return `${st}${who}`;
      })();
